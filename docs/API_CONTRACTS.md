...
| `text-linguistics` | Local model-backed linguistic interpretation | `text-core`, `text-lexical`, `text-models`, `text-transcripts`, `video-analysis-core`, `video-analysis-models` | Language detection, tokenizer routing/alignment, lemmatization, POS/morphology, chunks, dependencies, local model named entities, rule entity fallback, coreference, relations, events, discourse, topics, style, `TextAnalyzer` adapter | Applications, text pipelines, transcript analysis |
