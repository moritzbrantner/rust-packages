...
| `text-model-runtime` | Text model runtime helper contracts | `model-runtime`, `runtime-contracts`, `text-core` | Tokenization summaries, softmax helpers, text runtime request DTOs, non-executing local model helpers | Text model-backed crates, CLI model utilities, package UI runtime probes |
| `text-question-answering` | Question answering surface contracts | `model-runtime`, `runtime-contracts`, `text-core`, `text-classification` | QA model presets, question/context request DTOs, answer span responses, deterministic lexical fallback answers | Applications adding local-first question answering over documents and transcripts |
...
| `runtime-contracts` | Shared runtime surface DTOs | `serde`, `serde_json` | `PackageSurface`, `SurfaceOperation`, `SurfaceRequest`, `SurfaceResponse`, diagnostics, capabilities, job and artifact identifiers | Every CLI, server, WASM, and app wrapper |
| `runtime-artifacts` | Runtime artifact metadata and storage contracts | `runtime-contracts`, `serde`, `serde_json` | Artifact manifests, typed artifact references, storage descriptors, JSON serialization helpers | Runtime jobs, model commands, and package UI artifact reporting |
| `runtime-jobs` | Runtime job orchestration contracts | `jobs-core`, `runtime-artifacts`, `runtime-contracts` | Job request/status DTOs, progress events, artifact links, queue-friendly result records | Long-running runtime adapters, model operations, and UI job monitors |
| `jobs-core` | Reusable job state and progress contracts | `serde`, `serde_json`, `video-analysis-core` | Job IDs, specs, status transitions, progress snapshots, event records, in-memory job helpers | Runtime jobs, model materialization, and asynchronous package operations |
