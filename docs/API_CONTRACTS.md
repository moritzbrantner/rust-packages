...
| `moritzbrantner-audio-analysis-transcription` | Native audio/video transcription orchestration | `moritzbrantner-video-analysis-core`, `moritzbrantner-text-transcripts`, optional `moritzbrantner-model-runtime`, optional `moritzbrantner-audio-analysis-speakers` | ASR/VAD/alignment provider traits, Candle Whisper CPU/CUDA execution from local bundles with chunk/window-level timing, WAV native input normalization, energy VAD chunking, deterministic CTC alignment contracts with wav2vec2 bundle/tokenizer validation and typed unsupported emission execution, optional speaker diarization assignment, and external WhisperX compatibility import/execution | Real ASR workflows, transcript generation, alignment, diarization, and compatibility comparison |
