...
| Detection Stats and scene-list/result contracts | Both metrics/output stacks, with different contracts | `scenedetect-core` | Visual `MetricsStore` and result/report DTOs remain versioned compatibility and package-surface responses. |
| Boundary Candidate review | Only `scenedetect-core` | `scenedetect-core` | There is no visual Boundary Candidate review contract; visual metrics and output DTOs are not presented as one. |
...
| Scene CSV, stats CSV, HTML, and JSON artifacts | Both output stacks, with different existing shapes; visual JSON is a Detection Result rather than a canonical Scene List | `scenedetect-core` for canonical scene artifacts | Existing `video-analysis-output` shapes remain compatibility formats until a separately versioned migration. EDL, FCP7, FCPXML, OTIO, and qpfile output remain visual output ownership. |
| NDJSON scene events | Only `scenedetect-core` | `scenedetect-core` | There is no visual NDJSON writer or compatibility surface. |
