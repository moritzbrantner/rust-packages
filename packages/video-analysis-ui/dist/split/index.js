...
export function SplitPlanTable({ scenes, videoName = "video", template = "$VIDEO_NAME-Scene-$SCENE_NUMBER.mp4", }) {
    return (_jsx(Panel, { title: "Split Plan", description: `${scenes.length} output clips`, children: _jsx(DataTable, { rows: scenes, empty: "No scene clips", getRowKey: (scene, index) => `${sceneStartFrame(scene)}-${sceneEndFrame(scene)}-${index}`, columns: [
                {
                    key: "output",
                    header: "Output",
                    className: "font-medium text-zinc-950",
                    cell: (scene, index) => formatOutputName(template, videoName, sceneIndex(scene, index + 1), scene),
                },
                {
                    key: "start",
                    header: "Start",
