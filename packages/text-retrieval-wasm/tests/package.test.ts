...
test("text-retrieval-wasm package exports stable entrypoints", async () => {
...
test("runOperation searches documents with structured output", async () => {
  const entry = await import("../index.js");
  const result = assertStructuredResponse(
    await entry.runOperation({
      operation: "retrieval.search",
      input: {
        documents: [{ id: "doc-1", body: "rust retrieval search" }],
        query: "rust",
        mode: "full_text",
      },
    }),
