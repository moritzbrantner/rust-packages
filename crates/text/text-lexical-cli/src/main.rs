...
#[derive(Debug, Parser)]
#[command(
    name = "text-lexical-cli",
    version,
    about = "Thin CLI adapter for text-lexical"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            json,
            "text-lexical",
            &text_lexical_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "text-lexical command schema",
            &text_lexical_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload = serde_json::to_string(&text_lexical_cli::package_surface().operations)?;
            print_payload(json, "text-lexical operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = text_lexical_cli::run_operation(&operation, input)
