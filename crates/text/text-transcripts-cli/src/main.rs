...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        ...
        Command::Operations { json } => {
            let payload =
                serde_json::to_string(&text_transcripts_cli::package_surface().operations)?;
            print_payload(json, "text-transcripts operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = text_transcripts_cli::run_operation(&operation, input)
