...
pub const CLI_PACKAGE: &str = "text-nlp-tasks-cli";
/// Companion React app package name.
pub const APP_PACKAGE: &str = "text-nlp-tasks-app";
/// Companion WASM package name.
pub const WASM_PACKAGE: &str = "text-nlp-tasks-wasm";
...
pub fn response_for(method: &str, path: &str, body: &str) -> HttpResponse {
    match (method, path) {
        ...
        ("GET", "/api/operations") => json_response(
            200,
            "OK",
            serde_json::json!(text_nlp_tasks::surface::package_surface().operations),
        ),
...
fn package_metadata_value() -> serde_json::Value {
    let surface = text_nlp_tasks::surface::package_surface();
    serde_json::json!({
        "package": format!("{}-server", LIBRARY_CRATE),
...
fn schema_value() -> serde_json::Value {
    let operations = text_nlp_tasks::surface::package_surface()
        .operations
        .into_iter()
        .map(|operation| {
...
fn run_request(request: SurfaceRequest) -> HttpResponse {
    match text_nlp_tasks::surface::run_surface_operation(request) {
