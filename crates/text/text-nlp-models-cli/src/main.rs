...
#[derive(Debug, Parser)]
#[command(
    name = "text-nlp-models-cli",
    version,
    about = "Thin CLI adapter for text-nlp-models"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            json,
            "text-nlp-models",
            &text_nlp_models_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "text-nlp-models command schema",
            &text_nlp_models_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload =
                serde_json::to_string(&text_nlp_models_cli::package_surface().operations)?;
            print_payload(json, "text-nlp-models operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = text_nlp_models_cli::run_operation(&operation, input)
