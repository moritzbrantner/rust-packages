...
#[derive(Debug, Parser)]
#[command(
    name = "text-models-cli",
    version,
    about = "Thin CLI adapter for text-models"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            json,
            "text-models",
            &text_models_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "text-models command schema",
            &text_models_cli::command_schema_json(),
        ),
    }
}
