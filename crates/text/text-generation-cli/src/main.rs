...
#[derive(Debug, Parser)]
#[command(
    name = "text-generation-cli",
    version,
    about = "Thin CLI adapter for text-generation"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            json,
            "text-generation",
            &text_generation_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "text-generation command schema",
            &text_generation_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload =
                serde_json::to_string(&text_generation_cli::package_surface().operations)?;
            print_payload(json, "text-generation operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = text_generation_cli::run_operation(&operation, input)
