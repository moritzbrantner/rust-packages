...
#[derive(Debug, Parser)]
#[command(
    name = "text-model-runtime-server",
    version,
    about = "Thin HTTP API adapter for text-model-runtime"
)]
struct Args {
...
fn main() -> std::io::Result<()> {
    ...
    eprintln!(
        "text-model-runtime-server listening on http://{}",
        args.addr
    );
    text_model_runtime_server::serve(&args.addr)
}
