...
#[derive(Debug, Parser)]
#[command(
    name = "text-classification-cli",
    version,
    about = "Thin CLI adapter for text-classification"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            json,
            "text-classification",
            &text_classification_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "text-classification command schema",
            &text_classification_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload =
                serde_json::to_string(&text_classification_cli::package_surface().operations)?;
            print_payload(json, "text-classification operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = text_classification_cli::run_operation(&operation, input)
