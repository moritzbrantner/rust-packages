...
#[command(
    name = "text-core-cli",
    version,
    about = "Thin CLI adapter for text-core"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => {
            print_payload(json, "text-core", &text_core_cli::package_metadata_json())
        }
        Command::Schema { json } => print_payload(
            json,
            "text-core command schema",
            &text_core_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload = serde_json::to_string(&text_core_cli::package_surface().operations)?;
            print_payload(json, "text-core operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response =
                text_core_cli::run_operation(&operation, input).map_err(std::io::Error::other)?;
