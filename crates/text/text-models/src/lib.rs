...
impl TextEmbedderBackend for text_embeddings::HashedTextEmbedder {
    fn model_info(&self) -> EmbeddingModelInfo {
        EmbeddingModelInfo {
...
fn log_onnx_stage_timing(stage: &str, model_path: &Path, elapsed: std::time::Duration, ok: bool) {
    eprintln!(
        "text-models onnx timing: stage={stage} model={} elapsed_ms={} status={}",
        model_path.display(),
        elapsed.as_millis(),
        if ok { "ok" } else { "err" }
    );
...
fn log_onnx_stage_event(stage: &str, model_path: &Path, event: &str) {
    eprintln!(
        "text-models onnx timing: stage={stage} model={} event={event}",
        model_path.display()
    );
}
...
mod tests {
    ...
    fn log_slow_test_stage_timing(stage: &str, elapsed: std::time::Duration) {
        if onnx_timing_enabled() {
            eprintln!(
                "text-models slow onnx test timing: stage={stage} elapsed_ms={}",
                elapsed.as_millis()
            );
        }
    }
