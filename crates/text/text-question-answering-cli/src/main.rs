...
#[derive(Debug, Parser)]
#[command(
    name = "text-question-answering-cli",
    version,
    about = "Thin CLI adapter for text-question-answering"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            ...
            &text_question_answering_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "text-question-answering command schema",
            &text_question_answering_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload =
                serde_json::to_string(&text_question_answering_cli::package_surface().operations)?;
            print_payload(json, "text-question-answering operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = text_question_answering_cli::run_operation(&operation, input)
