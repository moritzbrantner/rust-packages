...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        ...
        Command::Operations { json } => {
            let payload = serde_json::to_string(&text_retrieval_cli::package_surface().operations)?;
            print_payload(json, "text-retrieval operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = text_retrieval_cli::run_operation(&operation, input)
