...
pub const SERVER_PACKAGE: &str = "text-retrieval-server";
...
pub fn command_schema_json() -> String {
    serde_json::json!({
        "commands": [
            ...
            {
                "name": "ingest",
                "description": "Ingest JSONL SearchDocument records into a persisted retrieval index."
            },
            {
                "name": "search",
                "description": "Search a persisted retrieval index."
            }
        ]
    })
    .to_string()
