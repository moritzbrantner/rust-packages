...
pub const LIBRARY_CRATE: &str = "text-question-answering";
/// Adapter surface kind.
pub const SURFACE_KIND: &str = "api";
/// Rust import path for the wrapped crate.
pub const LIBRARY_IMPORT: &str = "use text_question_answering";
/// Companion CLI package name.
pub const CLI_PACKAGE: &str = "text-question-answering-cli";
/// Companion React app package name.
pub const APP_PACKAGE: &str = "text-question-answering-app";
/// Companion WASM package name.
pub const WASM_PACKAGE: &str = "text-question-answering-wasm";
...
pub fn response_for(method: &str, path: &str, body: &str) -> HttpResponse {
    match (method, path) {
        ...
        ("GET", "/api/operations") => json_response(
            200,
            "OK",
            serde_json::json!(text_question_answering::surface::package_surface().operations),
        ),
...
fn schema_value() -> serde_json::Value {
    let operations = text_question_answering::surface::package_surface()
        .operations
        .into_iter()
        .map(|operation| {
...
fn run_request(request: SurfaceRequest) -> HttpResponse {
    match text_question_answering::surface::run_surface_operation(request) {
