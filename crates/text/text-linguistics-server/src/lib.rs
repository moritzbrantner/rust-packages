...
pub fn response_for(method: &str, path: &str, body: &str) -> HttpResponse {
    match (method, path) {
        ...
        ("GET", "/api/operations") => json_response(
            200,
            "OK",
            serde_json::json!(text_linguistics::surface::package_surface().operations),
        ),
...
fn schema_value() -> serde_json::Value {
    let operations = text_linguistics::surface::package_surface()
        .operations
        .into_iter()
        .map(|operation| {
...
fn run_request(request: SurfaceRequest) -> HttpResponse {
    match text_linguistics::surface::run_surface_operation(request) {
