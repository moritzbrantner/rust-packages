...
#[derive(Debug, Parser)]
#[command(
    name = "text-embeddings-cli",
    version,
    about = "Thin CLI adapter for text-embeddings"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            json,
            "text-embeddings",
            &text_embeddings_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "text-embeddings command schema",
            &text_embeddings_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload =
                serde_json::to_string(&text_embeddings_cli::package_surface().operations)?;
            print_payload(json, "text-embeddings operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = text_embeddings_cli::run_operation(&operation, input)
