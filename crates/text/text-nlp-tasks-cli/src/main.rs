...
#[derive(Debug, Parser)]
#[command(
    name = "text-nlp-tasks-cli",
    version,
    about = "Thin CLI adapter for text-nlp-tasks"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            json,
            "text-nlp-tasks",
            &text_nlp_tasks_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "text-nlp-tasks command schema",
            &text_nlp_tasks_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload = serde_json::to_string(&text_nlp_tasks_cli::package_surface().operations)?;
            print_payload(json, "text-nlp-tasks operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = text_nlp_tasks_cli::run_operation(&operation, input)
