...
#[command(
    name = "text-index-cli",
    version,
    about = "Thin CLI adapter for text-index"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => {
            print_payload(json, "text-index", &text_index_cli::package_metadata_json())
        }
        Command::Schema { json } => print_payload(
            json,
            "text-index command schema",
            &text_index_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload = serde_json::to_string(&text_index_cli::package_surface().operations)?;
            print_payload(json, "text-index operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response =
                text_index_cli::run_operation(&operation, input).map_err(std::io::Error::other)?;
