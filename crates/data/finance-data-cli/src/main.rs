...
#[derive(Debug, Parser)]
#[command(
    name = "finance-data-cli",
    version,
    about = "Thin CLI adapter for finance-data"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            json,
            "finance-data",
            &finance_data_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "finance-data command schema",
            &finance_data_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload = serde_json::to_string(&finance_data_cli::package_surface().operations)?;
            print_payload(json, "finance-data operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = finance_data_cli::run_operation(&operation, input)
