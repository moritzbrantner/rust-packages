...
#[derive(Debug, Parser)]
#[command(
    name = "data-inversion-core-cli",
    version,
    about = "Thin CLI adapter for data-inversion-core"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            json,
            "data-inversion-core",
            &data_inversion_core_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "data-inversion-core command schema",
            &data_inversion_core_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload =
                serde_json::to_string(&data_inversion_core_cli::package_surface().operations)?;
            print_payload(json, "data-inversion-core operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = data_inversion_core_cli::run_operation(&operation, input)
