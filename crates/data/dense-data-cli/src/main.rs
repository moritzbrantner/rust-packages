...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        ...
        Command::Operations { json } => {
            let payload = serde_json::to_string(&dense_data_cli::package_surface().operations)?;
            print_payload(json, "dense-data operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response =
                dense_data_cli::run_operation(&operation, input).map_err(std::io::Error::other)?;
