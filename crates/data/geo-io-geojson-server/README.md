# geo-io-geojson-server

Thin HTTP API adapter for `geo-io-geojson`.

## Legacy crate signpost

`moritzbrantner-geo-io-geojson-server` follows the Rust migration from
...
```bash
cargo run -p moenarch-geo-io-geojson-server -- --addr 127.0.0.1:3000
```
