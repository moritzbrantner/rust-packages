...
pub const LIBRARY_CRATE: &str = "geo-clustering";
/// Adapter surface kind.
pub const SURFACE_KIND: &str = "api";
/// Rust import path for the wrapped crate.
pub const LIBRARY_IMPORT: &str = "use geo_clustering";
/// Companion CLI package name.
pub const CLI_PACKAGE: &str = "geo-clustering-cli";
/// Companion React app package name.
pub const APP_PACKAGE: &str = "geo-clustering-app";
/// Companion WASM package name.
pub const WASM_PACKAGE: &str = "geo-clustering-wasm";
...
pub fn response_for(method: &str, path: &str, body: &str) -> HttpResponse {
    match (method, path) {
        ...
        ("GET", "/api/operations") => json_response(
            200,
            "OK",
            serde_json::json!(geo_clustering::surface::package_surface().operations),
        ),
...
fn schema_value() -> serde_json::Value {
    let operations = geo_clustering::surface::package_surface()
        .operations
        .into_iter()
        .map(|operation| {
...
fn run_request(request: SurfaceRequest) -> HttpResponse {
    match geo_clustering::surface::run_surface_operation(request) {
