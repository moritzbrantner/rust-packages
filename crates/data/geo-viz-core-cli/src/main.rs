...
#[derive(Debug, Parser)]
#[command(
    name = "geo-viz-core-cli",
    version,
    about = "Thin CLI adapter for geo-viz-core"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            json,
            "geo-viz-core",
            &geo_viz_core_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "geo-viz-core command schema",
            &geo_viz_core_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload = serde_json::to_string(&geo_viz_core_cli::package_surface().operations)?;
            print_payload(json, "geo-viz-core operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = geo_viz_core_cli::run_operation(&operation, input)
