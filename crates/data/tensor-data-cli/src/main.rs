...
#[derive(Debug, Parser)]
#[command(
    name = "tensor-data-cli",
    version,
    about = "Thin CLI adapter for tensor-data"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            json,
            "tensor-data",
            &tensor_data_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "tensor-data command schema",
            &tensor_data_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload = serde_json::to_string(&tensor_data_cli::package_surface().operations)?;
            print_payload(json, "tensor-data operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response =
                tensor_data_cli::run_operation(&operation, input).map_err(std::io::Error::other)?;
