...
#[derive(Debug, Parser)]
#[command(
    name = "geo-clustering-cli",
    version,
    about = "Thin CLI adapter for geo-clustering"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            json,
            "geo-clustering",
            &geo_clustering_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "geo-clustering command schema",
            &geo_clustering_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload = serde_json::to_string(&geo_clustering_cli::package_surface().operations)?;
            print_payload(json, "geo-clustering operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = geo_clustering_cli::run_operation(&operation, input)
