//! Library-owned runtime surface for `geo-data`.
...
use crate::{
    parse_geojson, to_geojson_geometry, Coordinate, GeoFeature, GeoFeatureCollection,
    GeoJsonDocument, Geometry,
};
    ...
    PackageSurface {
        ...
        operations: vec![
            ...
            operation(
                "geo.toGeoJson",
                "Geometry to GeoJSON",
                "Converts the crate Geometry JSON shape into a GeoJSON geometry object.",
...
pub fn run_surface_operation(request: SurfaceRequest) -> Result<SurfaceResponse, String> {
    ...
    let value = match request.operation.as_str() {
        ...
        "geo.toGeoJson" => to_geojson_value(parse_input(request.input)?)?,
        operation => {
            return Err(format!(
...
mod tests {
    ...
    fn converts_geometry_to_geojson() {
        let response = run_surface_operation(SurfaceRequest {
            operation: OperationId::new("geo.toGeoJson"),
