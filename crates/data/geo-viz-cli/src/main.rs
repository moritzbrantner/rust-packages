...
#[derive(Debug, Parser)]
#[command(name = "geo-viz-cli", version, about = "Thin CLI adapter for geo-viz")]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => {
            print_payload(json, "geo-viz", &geo_viz_cli::package_metadata_json())
        }
        Command::Schema { json } => print_payload(
            json,
            "geo-viz command schema",
            &geo_viz_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload = serde_json::to_string(&geo_viz_cli::package_surface().operations)?;
            print_payload(json, "geo-viz operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response =
                geo_viz_cli::run_operation(&operation, input).map_err(std::io::Error::other)?;
