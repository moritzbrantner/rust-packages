...
#[command(
    name = "geo-data-cli",
    version,
    about = "Thin CLI adapter for geo-data"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => {
            print_payload(json, "geo-data", &geo_data_cli::package_metadata_json())
        }
        Command::Schema { json } => print_payload(
            json,
            "geo-data command schema",
            &geo_data_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload = serde_json::to_string(&geo_data_cli::package_surface().operations)?;
            print_payload(json, "geo-data operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response =
                geo_data_cli::run_operation(&operation, input).map_err(std::io::Error::other)?;
