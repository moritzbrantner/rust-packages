...
pub const LIBRARY_CRATE: &str = "geo-data";
/// Adapter surface kind.
pub const SURFACE_KIND: &str = "api";
/// Rust import path for the wrapped crate.
pub const LIBRARY_IMPORT: &str = "use geo_data";
/// Companion CLI package name.
pub const CLI_PACKAGE: &str = "geo-data-cli";
/// Companion React app package name.
pub const APP_PACKAGE: &str = "geo-data-app";
/// Companion WASM package name.
pub const WASM_PACKAGE: &str = "geo-data-wasm";
...
pub fn response_for(method: &str, path: &str, body: &str) -> HttpResponse {
    match (method, path) {
        ...
        ("GET", "/api/operations") => json_response(
            200,
            "OK",
            serde_json::json!(geo_data::surface::package_surface().operations),
        ),
...
fn package_metadata_value() -> serde_json::Value {
    let surface = geo_data::surface::package_surface();
    serde_json::json!({
        "package": format!("{}-server", LIBRARY_CRATE),
...
fn schema_value() -> serde_json::Value {
    let operations = geo_data::surface::package_surface()
        .operations
        .into_iter()
        .map(|operation| {
...
fn run_request(request: SurfaceRequest) -> HttpResponse {
    match geo_data::surface::run_surface_operation(request) {
