...
#[command(
    name = "geo-core-cli",
    version,
    about = "Thin CLI adapter for geo-core"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => {
            print_payload(json, "geo-core", &geo_core_cli::package_metadata_json())
        }
        Command::Schema { json } => print_payload(
            json,
            "geo-core command schema",
            &geo_core_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload = serde_json::to_string(&geo_core_cli::package_surface().operations)?;
            print_payload(json, "geo-core operations", &payload);
        }
        Command::Run {
            operation,
            json,
