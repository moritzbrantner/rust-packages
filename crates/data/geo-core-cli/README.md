...
Thin command-line adapter for `geo-core`.

## Legacy crate signpost

`moritzbrantner-geo-core-cli` follows the Rust migration from
...
```bash
cargo run -p moenarch-geo-core-cli -- operations --json
cargo run -p moenarch-geo-core-cli -- run --operation describe --json '{"includeOperations":true}'
```
