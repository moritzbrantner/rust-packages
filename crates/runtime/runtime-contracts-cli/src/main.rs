...
#[derive(Debug, Parser)]
#[command(
    name = "runtime-contracts-cli",
    version,
    about = "Thin CLI adapter for runtime-contracts"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            json,
            "runtime-contracts",
            &runtime_contracts_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "runtime-contracts command schema",
            &runtime_contracts_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload =
                serde_json::to_string(&runtime_contracts_cli::package_surface().operations)?;
            print_payload(json, "runtime-contracts operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = runtime_contracts_cli::run_operation(&operation, input)
