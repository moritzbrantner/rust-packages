...
#[derive(Debug, Parser)]
#[command(
    name = "runtime-jobs-cli",
    version,
    about = "Thin CLI adapter for runtime-jobs"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            json,
            "runtime-jobs",
            &runtime_jobs_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "runtime-jobs command schema",
            &runtime_jobs_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload = serde_json::to_string(&runtime_jobs_cli::package_surface().operations)?;
            print_payload(json, "runtime-jobs operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = runtime_jobs_cli::run_operation(&operation, input)
