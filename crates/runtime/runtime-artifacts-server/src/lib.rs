...
pub const LIBRARY_CRATE: &str = "runtime-artifacts";
/// Adapter surface kind.
pub const SURFACE_KIND: &str = "api";
/// Rust import path for the wrapped crate.
pub const LIBRARY_IMPORT: &str = "use runtime_artifacts";
/// Companion CLI package name.
pub const CLI_PACKAGE: &str = "runtime-artifacts-cli";
/// Companion React app package name.
pub const APP_PACKAGE: &str = "runtime-artifacts-app";
/// Companion WASM package name.
pub const WASM_PACKAGE: &str = "runtime-artifacts-wasm";
...
pub fn response_for(method: &str, path: &str, body: &str) -> HttpResponse {
    match (method, path) {
        ...
        ("GET", "/api/operations") => json_response(
            200,
            "OK",
            serde_json::json!(runtime_artifacts::surface::package_surface().operations),
        ),
...
fn schema_value() -> serde_json::Value {
    let operations = runtime_artifacts::surface::package_surface()
        .operations
        .into_iter()
        .map(|operation| {
...
fn run_request(request: SurfaceRequest) -> HttpResponse {
    match runtime_artifacts::surface::run_surface_operation(request) {
