...
#[derive(Debug, Parser)]
#[command(
    name = "model-runtime-cli",
    version,
    about = "Thin CLI adapter for model-runtime"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            json,
            "model-runtime",
            &model_runtime_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "model-runtime command schema",
            &model_runtime_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload = serde_json::to_string(&model_runtime_cli::package_surface().operations)?;
            print_payload(json, "model-runtime operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = model_runtime_cli::run_operation(&operation, input)
