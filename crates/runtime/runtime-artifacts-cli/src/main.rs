...
#[derive(Debug, Parser)]
#[command(
    name = "runtime-artifacts-cli",
    version,
    about = "Thin CLI adapter for runtime-artifacts"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            json,
            "runtime-artifacts",
            &runtime_artifacts_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "runtime-artifacts command schema",
            &runtime_artifacts_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload =
                serde_json::to_string(&runtime_artifacts_cli::package_surface().operations)?;
            print_payload(json, "runtime-artifacts operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = runtime_artifacts_cli::run_operation(&operation, input)
