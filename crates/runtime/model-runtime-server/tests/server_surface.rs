...
fn package_endpoint_reports_wrapped_library() {
    let response = model_runtime_server::response_for("GET", "/api/package", "");
    assert_eq!(response.status_code, 200);
    assert!(response.body.contains("model-runtime"));
}

#[test]
fn run_endpoint_calls_library_surface() {
    let response = model_runtime_server::response_for(
        "POST",
        "/api/run",
