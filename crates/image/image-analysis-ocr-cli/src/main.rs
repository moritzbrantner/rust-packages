...
#[derive(Debug, Parser)]
#[command(
    name = "image-analysis-ocr-cli",
    version,
    about = "Thin CLI adapter for image-analysis-ocr"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            json,
            "image-analysis-ocr",
            &image_analysis_ocr_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "image-analysis-ocr command schema",
            &image_analysis_ocr_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload =
                serde_json::to_string(&image_analysis_ocr_cli::package_surface().operations)?;
            print_payload(json, "image-analysis-ocr operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = image_analysis_ocr_cli::run_operation(&operation, input)
