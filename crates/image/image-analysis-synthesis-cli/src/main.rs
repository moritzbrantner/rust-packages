...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        ...
        Command::Operations { json } => {
            let payload =
                serde_json::to_string(&image_analysis_synthesis_cli::package_surface().operations)?;
            print_payload(json, "image-analysis-synthesis operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = image_analysis_synthesis_cli::run_operation(&operation, input)
