...
#[derive(Debug, Parser)]
#[command(
    name = "image-analysis-embeddings-cli",
    version,
    about = "Thin CLI adapter for image-analysis-embeddings"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            ...
            &image_analysis_embeddings_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "image-analysis-embeddings command schema",
            &image_analysis_embeddings_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload = serde_json::to_string(
                &image_analysis_embeddings_cli::package_surface().operations,
            )?;
            print_payload(json, "image-analysis-embeddings operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = image_analysis_embeddings_cli::run_operation(&operation, input)
