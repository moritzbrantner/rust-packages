...
#[derive(Debug, Parser)]
#[command(
    name = "image-analysis-io-cli",
    version,
    about = "Thin CLI adapter for image-analysis-io"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            json,
            "image-analysis-io",
            &image_analysis_io_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "image-analysis-io command schema",
            &image_analysis_io_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload =
                serde_json::to_string(&image_analysis_io_cli::package_surface().operations)?;
            print_payload(json, "image-analysis-io operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = image_analysis_io_cli::run_operation(&operation, input)
