...
pub const LIBRARY_CRATE: &str = "image-analysis-synthesis";
/// Adapter surface kind.
pub const SURFACE_KIND: &str = "api";
/// Rust import path for the wrapped crate.
pub const LIBRARY_IMPORT: &str = "use image_analysis_synthesis";
/// Companion CLI package name.
pub const CLI_PACKAGE: &str = "image-analysis-synthesis-cli";
/// Companion React app package name.
pub const APP_PACKAGE: &str = "image-analysis-synthesis-app";
/// Companion WASM package name.
pub const WASM_PACKAGE: &str = "image-analysis-synthesis-wasm";
...
pub fn response_for(method: &str, path: &str, body: &str) -> HttpResponse {
    match (method, path) {
        ...
        ("GET", "/api/operations") => json_response(
            200,
            "OK",
            serde_json::json!(image_analysis_synthesis::surface::package_surface().operations),
        ),
...
fn package_metadata_value() -> serde_json::Value {
    let surface = image_analysis_synthesis::surface::package_surface();
    serde_json::json!({
        "package": format!("{}-server", LIBRARY_CRATE),
...
fn schema_value() -> serde_json::Value {
    let operations = image_analysis_synthesis::surface::package_surface()
        .operations
        .into_iter()
        .map(|operation| {
...
fn run_request(request: SurfaceRequest) -> HttpResponse {
    match image_analysis_synthesis::surface::run_surface_operation(request) {
