...
struct YoutubeVideoReport {
    ...
    audio: AudioReport,
    text: TextReport,
    data_buckets: Vec<DataBucketReport>,
}
...
struct TranscriptionReport {
    status: String,
    text: Option<String>,
    segments: Vec<TranscriptSegmentReport>,
    message: Option<String>,
}

#[derive(Debug, Serialize, Clone)]
struct TranscriptSegmentReport {
    index: u64,
...
#[derive(Debug, Serialize)]
struct DataBucketReport {
    bucket_index: u64,
    records: u64,
    estimated_bytes: u64,
    streams: Vec<StreamBucketReport>,
}

#[derive(Debug, Serialize)]
struct StreamBucketReport {
    stream_id: String,
    records: u64,
    estimated_bytes: u64,
    payload_counts: BTreeMap<String, u64>,
    video_frames: u64,
    audio_frames: u64,
    text_segments: u64,
}
...
fn main() -> Result<()> {
    let cli = Cli::parse();
    match cli.command {
        CommandKind::YoutubeVideo(args) => run_youtube_video_use_case(args)?,
    }
    Ok(())
}

fn run_youtube_video_use_case(args: YoutubeVideoArgs) -> Result<()> {
    ...
    let report = YoutubeVideoReport {
        use_case: "youtube_video_analysis",
        source: SourceReport {
            url: args.url.clone(),
...
fn build_video_pipeline(args: &YoutubeVideoArgs) -> Result<RealtimeVideoPipeline> {
...
fn analyze_transcript_text(
    ...
    let mut builder = video_analysis_core::TextPipeline::builder()
        .analyzer(TranscriptHeuristicAnalyzer::default());
