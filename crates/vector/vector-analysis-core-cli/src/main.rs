...
#[derive(Debug, Parser)]
#[command(
    name = "vector-analysis-core-cli",
    version,
    about = "Thin CLI adapter for vector-analysis-core"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            ...
            &vector_analysis_core_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "vector-analysis-core command schema",
            &vector_analysis_core_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload =
                serde_json::to_string(&vector_analysis_core_cli::package_surface().operations)?;
            print_payload(json, "vector-analysis-core operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = vector_analysis_core_cli::run_operation(&operation, input)
