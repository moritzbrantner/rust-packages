...
pub const LIBRARY_IMPORT: &str = "use comfyui_models";
/// Companion CLI package name.
pub const CLI_PACKAGE: &str = "comfyui-models-cli";
/// Companion React app package name.
pub const APP_PACKAGE: &str = "comfyui-models-app";
/// Companion WASM package name.
pub const WASM_PACKAGE: &str = "comfyui-models-wasm";
...
pub fn response_for(method: &str, path: &str, body: &str) -> HttpResponse {
    match (method, path) {
        ...
        ("GET", "/api/operations") => json_response(
            200,
            "OK",
            serde_json::json!(comfyui_models::surface::package_surface().operations),
        ),
...
fn package_metadata_value() -> serde_json::Value {
    let surface = comfyui_models::surface::package_surface();
    serde_json::json!({
        "package": format!("{}-server", LIBRARY_CRATE),
...
fn schema_value() -> serde_json::Value {
    let operations = comfyui_models::surface::package_surface()
        .operations
        .into_iter()
        .map(|operation| {
...
fn run_request(request: SurfaceRequest) -> HttpResponse {
    match comfyui_models::surface::run_surface_operation(request) {
