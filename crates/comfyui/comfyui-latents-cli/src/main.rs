...
#[derive(Debug, Parser)]
#[command(
    name = "comfyui-latents-cli",
    version,
    about = "Thin CLI adapter for comfyui-latents"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            json,
            "comfyui-latents",
            &comfyui_latents_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "comfyui-latents command schema",
            &comfyui_latents_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload =
                serde_json::to_string(&comfyui_latents_cli::package_surface().operations)?;
            print_payload(json, "comfyui-latents operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = comfyui_latents_cli::run_operation(&operation, input)
