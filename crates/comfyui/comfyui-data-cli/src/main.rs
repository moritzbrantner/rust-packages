...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        ...
        Command::Operations { json } => {
            let payload = serde_json::to_string(&comfyui_data_cli::package_surface().operations)?;
            print_payload(json, "comfyui-data operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = comfyui_data_cli::run_operation(&operation, input)
