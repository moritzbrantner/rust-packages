...
#[derive(Debug, Parser)]
#[command(
    name = "comfyui-models-cli",
    version,
    about = "Thin CLI adapter for comfyui-models"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            json,
            "comfyui-models",
            &comfyui_models_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "comfyui-models command schema",
            &comfyui_models_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload = serde_json::to_string(&comfyui_models_cli::package_surface().operations)?;
            print_payload(json, "comfyui-models operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = comfyui_models_cli::run_operation(&operation, input)
