...
pub const LIBRARY_CRATE: &str = "three-d-processing-core";
/// Adapter surface kind.
pub const SURFACE_KIND: &str = "api";
/// Rust import path for the wrapped crate.
pub const LIBRARY_IMPORT: &str = "use three_d_processing_core";
/// Companion CLI package name.
pub const CLI_PACKAGE: &str = "three-d-processing-core-cli";
/// Companion React app package name.
pub const APP_PACKAGE: &str = "three-d-processing-core-app";
/// Companion WASM package name.
pub const WASM_PACKAGE: &str = "three-d-processing-core-wasm";
...
pub fn response_for(method: &str, path: &str, body: &str) -> HttpResponse {
    match (method, path) {
        ...
        ("GET", "/api/operations") => json_response(
            200,
            "OK",
            serde_json::json!(three_d_processing_core::surface::package_surface().operations),
        ),
...
fn package_metadata_value() -> serde_json::Value {
    let surface = three_d_processing_core::surface::package_surface();
    serde_json::json!({
        "package": format!("{}-server", LIBRARY_CRATE),
...
fn schema_value() -> serde_json::Value {
    let operations = three_d_processing_core::surface::package_surface()
        .operations
        .into_iter()
        .map(|operation| {
...
fn run_request(request: SurfaceRequest) -> HttpResponse {
    match three_d_processing_core::surface::run_surface_operation(request) {
