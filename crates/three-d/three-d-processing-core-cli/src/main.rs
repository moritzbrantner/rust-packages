...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        ...
        Command::Operations { json } => {
            let payload =
                serde_json::to_string(&three_d_processing_core_cli::package_surface().operations)?;
            print_payload(json, "three-d-processing-core operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = three_d_processing_core_cli::run_operation(&operation, input)
