...
#[derive(Debug, Parser)]
#[command(
    name = "three-d-processing-mesh-cli",
    version,
    about = "Thin CLI adapter for three-d-processing-mesh"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            ...
            &three_d_processing_mesh_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "three-d-processing-mesh command schema",
            &three_d_processing_mesh_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload =
                serde_json::to_string(&three_d_processing_mesh_cli::package_surface().operations)?;
            print_payload(json, "three-d-processing-mesh operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = three_d_processing_mesh_cli::run_operation(&operation, input)
