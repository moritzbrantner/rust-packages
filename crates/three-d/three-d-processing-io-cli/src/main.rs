...
#[derive(Debug, Parser)]
#[command(
    name = "three-d-processing-io-cli",
    version,
    about = "Thin CLI adapter for three-d-processing-io"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            ...
            &three_d_processing_io_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "three-d-processing-io command schema",
            &three_d_processing_io_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload =
                serde_json::to_string(&three_d_processing_io_cli::package_surface().operations)?;
            print_payload(json, "three-d-processing-io operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = three_d_processing_io_cli::run_operation(&operation, input)
