...
pub const LIBRARY_CRATE: &str = "finance-statistics";
/// Adapter surface kind.
pub const SURFACE_KIND: &str = "api";
/// Rust import path for the wrapped crate.
pub const LIBRARY_IMPORT: &str = "use finance_statistics";
/// Companion CLI package name.
pub const CLI_PACKAGE: &str = "finance-statistics-cli";
/// Companion React app package name.
pub const APP_PACKAGE: &str = "finance-statistics-app";
/// Companion WASM package name.
pub const WASM_PACKAGE: &str = "finance-statistics-wasm";
...
pub fn response_for(method: &str, path: &str, body: &str) -> HttpResponse {
    match (method, path) {
        ...
        ("GET", "/api/operations") => json_response(
            200,
            "OK",
            serde_json::json!(finance_statistics::surface::package_surface().operations),
        ),
...
fn schema_value() -> serde_json::Value {
    let operations = finance_statistics::surface::package_surface()
        .operations
        .into_iter()
        .map(|operation| {
...
fn run_request(request: SurfaceRequest) -> HttpResponse {
    match finance_statistics::surface::run_surface_operation(request) {
