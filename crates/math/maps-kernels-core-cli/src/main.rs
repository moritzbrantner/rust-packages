...
#[derive(Debug, Parser)]
#[command(
    name = "maps-kernels-core-cli",
    version,
    about = "Thin CLI adapter for maps-kernels-core"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            json,
            "maps-kernels-core",
            &maps_kernels_core_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "maps-kernels-core command schema",
            &maps_kernels_core_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload =
                serde_json::to_string(&maps_kernels_core_cli::package_surface().operations)?;
            print_payload(json, "maps-kernels-core operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = maps_kernels_core_cli::run_operation(&operation, input)
