...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        ...
        Command::Operations { json } => {
            let payload = serde_json::to_string(&math_linear_cli::package_surface().operations)?;
            print_payload(json, "math-linear operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response =
                math_linear_cli::run_operation(&operation, input).map_err(std::io::Error::other)?;
