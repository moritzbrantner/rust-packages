...
pub const LIBRARY_CRATE: &str = "maps-kernels-core";
/// Adapter surface kind.
pub const SURFACE_KIND: &str = "api";
/// Rust import path for the wrapped crate.
pub const LIBRARY_IMPORT: &str = "use maps_kernels_core";
/// Companion CLI package name.
pub const CLI_PACKAGE: &str = "maps-kernels-core-cli";
/// Companion React app package name.
pub const APP_PACKAGE: &str = "maps-kernels-core-app";
/// Companion WASM package name.
pub const WASM_PACKAGE: &str = "maps-kernels-core-wasm";
...
pub fn response_for(method: &str, path: &str, body: &str) -> HttpResponse {
    match (method, path) {
        ...
        ("GET", "/api/operations") => json_response(
            200,
            "OK",
            serde_json::json!(maps_kernels_core::surface::package_surface().operations),
        ),
...
fn schema_value() -> serde_json::Value {
    let operations = maps_kernels_core::surface::package_surface()
        .operations
        .into_iter()
        .map(|operation| {
...
fn run_request(request: SurfaceRequest) -> HttpResponse {
    match maps_kernels_core::surface::run_surface_operation(request) {
