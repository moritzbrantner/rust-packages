//! WASM bindings for `video-analysis-ffmpeg`.
...
pub fn package_surface() -> Result<JsValue, JsValue> {
    serde_wasm_bindgen::to_value(&video_analysis_ffmpeg::surface::package_surface())
...
pub fn run_operation(request: JsValue) -> Result<JsValue, JsValue> {
    ...
    let response =
        video_analysis_ffmpeg::surface::run_surface_operation(request).map_err(into_js_error)?;
