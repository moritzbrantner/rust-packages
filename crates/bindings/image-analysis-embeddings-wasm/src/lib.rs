//! WASM bindings for `image-analysis-embeddings`.
...
pub fn package_surface() -> Result<JsValue, JsValue> {
    serde_wasm_bindgen::to_value(&image_analysis_embeddings::surface::package_surface())
...
pub fn run_operation(request: JsValue) -> Result<JsValue, JsValue> {
    ...
    let response = image_analysis_embeddings::surface::run_surface_operation(request)
        .map_err(into_js_error)?;
