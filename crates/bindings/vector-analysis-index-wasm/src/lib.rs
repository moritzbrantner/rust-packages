//! WASM bindings for `vector-analysis-index`.
...
pub fn package_surface() -> Result<JsValue, JsValue> {
    serde_wasm_bindgen::to_value(&vector_analysis_index::surface::package_surface())
...
pub fn run_operation(request: JsValue) -> Result<JsValue, JsValue> {
    ...
    let response =
        vector_analysis_index::surface::run_surface_operation(request).map_err(into_js_error)?;
