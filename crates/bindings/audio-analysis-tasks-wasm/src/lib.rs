//! WASM bindings for `audio-analysis-tasks`.
...
pub fn package_surface() -> Result<JsValue, JsValue> {
    serde_wasm_bindgen::to_value(&audio_analysis_tasks::surface::package_surface())
...
pub fn run_operation(request: JsValue) -> Result<JsValue, JsValue> {
    ...
    let response =
        audio_analysis_tasks::surface::run_surface_operation(request).map_err(into_js_error)?;
