//! WASM bindings for `audio-analysis-recognition`.
...
pub fn package_surface() -> Result<JsValue, JsValue> {
    serde_wasm_bindgen::to_value(&audio_analysis_recognition::surface::package_surface())
...
pub fn run_operation(request: JsValue) -> Result<JsValue, JsValue> {
    ...
    let response = audio_analysis_recognition::surface::run_surface_operation(request)
        .map_err(into_js_error)?;
...
mod tests {
    ...
    fn wrapped_surface_has_operations() {
        let surface = audio_analysis_recognition::surface::package_surface();
        assert_eq!(surface.library, "moenarch-audio-analysis-recognition");
        ...
        let operation = surface
            ...
            .unwrap();
        let response = audio_analysis_recognition::surface::run_surface_operation(
