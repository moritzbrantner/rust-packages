//! WASM bindings for `text-nlp-tasks`.
...
pub fn package_surface() -> Result<JsValue, JsValue> {
    serde_wasm_bindgen::to_value(&text_nlp_tasks::surface::package_surface()).map_err(into_js_error)
}
...
pub fn run_operation(request: JsValue) -> Result<JsValue, JsValue> {
    ...
    let response =
        text_nlp_tasks::surface::run_surface_operation(request).map_err(into_js_error)?;
