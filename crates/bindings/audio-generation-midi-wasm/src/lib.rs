//! WASM bindings for `audio-generation-midi`.
...
pub fn package_surface() -> Result<JsValue, JsValue> {
    serde_wasm_bindgen::to_value(&audio_generation_midi::surface::package_surface())
...
pub fn run_operation(request: JsValue) -> Result<JsValue, JsValue> {
    ...
    let response =
        audio_generation_midi::surface::run_surface_operation(request).map_err(into_js_error)?;
...
mod tests {
    ...
    fn wrapped_surface_has_operations() {
        let surface = audio_generation_midi::surface::package_surface();
        ...
        let operation = surface
            ...
            .unwrap();
        let response =
            audio_generation_midi::surface::run_surface_operation(runtime_core::SurfaceRequest {
