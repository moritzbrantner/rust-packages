...
pub fn package_surface() -> Result<JsValue, JsValue> {
    serde_wasm_bindgen::to_value(&geo_data::surface::package_surface()).map_err(into_js_error)
}
...
pub fn run_operation(request: JsValue) -> Result<JsValue, JsValue> {
    ...
    let response = geo_data::surface::run_surface_operation(request).map_err(into_js_error)?;
