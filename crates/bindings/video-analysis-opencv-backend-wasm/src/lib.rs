//! WASM bindings for `video-analysis-opencv-backend`.
...
pub fn package_surface() -> Result<JsValue, JsValue> {
    serde_wasm_bindgen::to_value(&video_analysis_opencv_backend::surface::package_surface())
...
pub fn run_operation(request: JsValue) -> Result<JsValue, JsValue> {
    ...
    let response = video_analysis_opencv_backend::surface::run_surface_operation(request)
        .map_err(into_js_error)?;
