//! WASM bindings for `audio-analysis-fourier`.
...
pub fn package_surface() -> Result<JsValue, JsValue> {
    serde_wasm_bindgen::to_value(&audio_analysis_fourier::surface::package_surface())
...
pub fn run_operation(request: JsValue) -> Result<JsValue, JsValue> {
    ...
    let response =
        audio_analysis_fourier::surface::run_surface_operation(request).map_err(into_js_error)?;
...
mod tests {
    ...
    fn wrapped_surface_has_operations() {
        ...
        assert_eq!(surface.library, "moritzbrantner-audio-analysis-fourier");
        ...
        let operation = surface
            ...
            .unwrap();
        let response =
            audio_analysis_fourier::surface::run_surface_operation(runtime_core::SurfaceRequest {
