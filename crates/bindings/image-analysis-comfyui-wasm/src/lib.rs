//! WASM bindings for `image-analysis-comfyui`.
...
pub fn package_surface() -> Result<JsValue, JsValue> {
    serde_wasm_bindgen::to_value(&image_analysis_comfyui::surface::package_surface())
...
pub fn run_operation(request: JsValue) -> Result<JsValue, JsValue> {
    ...
    let response =
        image_analysis_comfyui::surface::run_surface_operation(request).map_err(into_js_error)?;
...
mod tests {
    ...
    fn wrapped_surface_has_operations() {
        ...
        assert_eq!(surface.library, "moritzbrantner-image-analysis-comfyui");
