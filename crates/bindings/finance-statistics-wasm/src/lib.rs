//! WASM bindings for `finance-statistics`.
...
pub fn package_surface() -> Result<JsValue, JsValue> {
    serde_wasm_bindgen::to_value(&finance_statistics::surface::package_surface())
...
pub fn run_operation(request: JsValue) -> Result<JsValue, JsValue> {
    ...
    let response =
        finance_statistics::surface::run_surface_operation(request).map_err(into_js_error)?;
