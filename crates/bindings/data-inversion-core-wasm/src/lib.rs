//! WASM bindings for `data-inversion-core`.
...
pub fn package_surface() -> Result<JsValue, JsValue> {
    serde_wasm_bindgen::to_value(&data_inversion_core::surface::package_surface())
...
pub fn run_operation(request: JsValue) -> Result<JsValue, JsValue> {
    ...
    let response =
        data_inversion_core::surface::run_surface_operation(request).map_err(into_js_error)?;
