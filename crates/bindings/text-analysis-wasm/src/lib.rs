//! WASM bindings for `text-analysis`.
...
pub fn package_surface() -> Result<JsValue, JsValue> {
    serde_wasm_bindgen::to_value(&text_analysis::surface::package_surface()).map_err(into_js_error)
}
...
pub fn run_operation(request: JsValue) -> Result<JsValue, JsValue> {
    ...
    let response = text_analysis::surface::run_surface_operation(request).map_err(into_js_error)?;
...
mod tests {
    ...
    fn wrapped_surface_has_operations() {
        ...
        assert_eq!(surface.library, "moritzbrantner-text-analysis");
