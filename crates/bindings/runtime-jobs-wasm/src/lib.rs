//! WASM bindings for `runtime-jobs`.
...
pub fn package_surface() -> Result<JsValue, JsValue> {
    serde_wasm_bindgen::to_value(&runtime_jobs::surface::package_surface()).map_err(into_js_error)
}
...
pub fn run_operation(request: JsValue) -> Result<JsValue, JsValue> {
    ...
    let response = runtime_jobs::surface::run_surface_operation(request).map_err(into_js_error)?;
