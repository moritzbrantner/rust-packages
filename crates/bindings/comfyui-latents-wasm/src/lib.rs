//! WASM bindings for `comfyui-latents`.
...
pub fn package_surface() -> Result<JsValue, JsValue> {
    serde_wasm_bindgen::to_value(&comfyui_latents::surface::package_surface())
...
pub fn run_operation(request: JsValue) -> Result<JsValue, JsValue> {
    ...
    let response =
        comfyui_latents::surface::run_surface_operation(request).map_err(into_js_error)?;
