//! WASM bindings for `text-transcripts`.
...
pub fn package_surface() -> Result<JsValue, JsValue> {
    serde_wasm_bindgen::to_value(&text_transcripts::surface::package_surface())
...
pub fn run_operation(request: JsValue) -> Result<JsValue, JsValue> {
    ...
    let response =
        text_transcripts::surface::run_surface_operation(request).map_err(into_js_error)?;
