//! WASM bindings for `three-d-processing-mesh`.
...
pub fn package_surface() -> Result<JsValue, JsValue> {
    serde_wasm_bindgen::to_value(&three_d_processing_mesh::surface::package_surface())
...
pub fn run_operation(request: JsValue) -> Result<JsValue, JsValue> {
    ...
    let response =
        three_d_processing_mesh::surface::run_surface_operation(request).map_err(into_js_error)?;
...
mod tests {
    ...
    fn wrapped_surface_has_operations() {
        ...
        assert_eq!(surface.library, "moritzbrantner-three-d-processing-mesh");
