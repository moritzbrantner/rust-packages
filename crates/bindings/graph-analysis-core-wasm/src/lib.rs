//! WASM bindings for `graph-analysis-core`.
...
pub fn package_surface() -> Result<JsValue, JsValue> {
    serde_wasm_bindgen::to_value(&graph_analysis_core::surface::package_surface())
...
pub fn run_operation(request: JsValue) -> Result<JsValue, JsValue> {
    ...
    let response =
        graph_analysis_core::surface::run_surface_operation(request).map_err(into_js_error)?;
