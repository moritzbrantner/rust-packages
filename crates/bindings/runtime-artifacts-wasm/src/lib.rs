//! WASM bindings for `runtime-artifacts`.
...
pub fn package_surface() -> Result<JsValue, JsValue> {
    serde_wasm_bindgen::to_value(&runtime_artifacts::surface::package_surface())
...
pub fn run_operation(request: JsValue) -> Result<JsValue, JsValue> {
    ...
    let response =
        runtime_artifacts::surface::run_surface_operation(request).map_err(into_js_error)?;
