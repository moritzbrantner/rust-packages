...
use media_core::{AnalysisEvent, MediaRange, Timebase, Timestamp};
...
pub enum AnnotationError {
    ...
    #[error(transparent)]
    Media(#[from] media_core::DetectError),
...
impl TimebaseWire {
    fn into_timebase(self) -> media_core::Result<Timebase> {
...
impl TimestampWire {
    fn into_timestamp(self) -> media_core::Result<Timestamp> {
...
impl AnnotationTimingWire {
    fn into_timing(self) -> media_core::Result<AnnotationTiming> {
        match self {
...
impl AnnotationDataset {
    ...
    pub fn at(&self, timestamp: Timestamp) -> Result<Vec<&MediaAnnotation>> {
        ...
        for annotation in &self.annotations {
            if annotation
                .timing
                .is_some_and(|timing| timing.contains(timestamp).unwrap_or(false))
            {
    ...
    pub fn overlapping(&self, range: MediaRange) -> Result<Vec<&MediaAnnotation>> {
        ...
        for annotation in &self.annotations {
            if annotation
                .timing
                .is_some_and(|timing| timing.overlaps(range).unwrap_or(false))
            {
                matches.push(annotation);
            }
        }
...
mod tests {
    ...
    fn temporal_queries_compare_different_timebases_exactly() {
        ...
        let query = MediaRange::new(
            timestamp(900, 1, 1_000),
            timestamp(2, 1, 1),
        )
        .unwrap();
        let ids = dataset
    ...
    fn analysis_events_adapt_without_losing_timing_or_score() {
        ...
        assert_eq!(annotation.start_timestamp(), Some(timestamp(1, 1, 1)));
        assert_eq!(
    ...
    fn duplicate_ids_are_rejected() {
        let mut dataset = AnnotationDataset::new();
        dataset
            .push(MediaAnnotation::new("same", "one"))
            .unwrap();
        assert!(dataset
            .push(MediaAnnotation::new("same", "two"))
            .is_err());
    }
}
