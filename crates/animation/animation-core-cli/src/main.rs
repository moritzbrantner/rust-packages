...
#[derive(Debug, Parser)]
#[command(
    name = "animation-core-cli",
    version,
    about = "Thin CLI adapter for animation-core"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            json,
            "animation-core",
            &animation_core_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "animation-core command schema",
            &animation_core_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload = serde_json::to_string(&animation_core_cli::package_surface().operations)?;
            print_payload(json, "animation-core operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = animation_core_cli::run_operation(&operation, input)
