...
impl PyannoteCommunityDiarizer {
    ...
    pub fn from_config(config: PyannoteCommunityDiarizationConfig) -> Result<Self> {
        ...
        let segmentation = OnnxSession::from_file_cpu_single_threaded(
            &resolved.segmentation_model_path,
        )
        .map_err(map_onnx_session_error)?;
        let embedding =
            OnnxSession::from_file_cpu_single_threaded(&resolved.embedding_model_path)
                .map_err(map_onnx_session_error)?;
        Ok(Self {
            manifest,
            segmentation,
    ...
    pub fn diarize(
        ...
        let speaker_embeddings = if self.config.return_speaker_embeddings {
            Some(speaker_embedding_map(&self.manifest, &embeddings, &assignments)?)
        } else {
            None
        };
        let diagnostics = vec![
            ...
            format!("pyannoteSegmentationWindows={}", segmentations.windows.len()),
            ...
            format!("pyannotePldaModel={}", self.config.plda_model_path.display()),
            format!(
    ...
    fn run_segmentations(&mut self, samples: &[f32]) -> Result<SegmentationBatch> {
        ...
        loop {
            ...
            let outputs = self
                ...
                .run(vec![tensor])
                .map_err(map_onnx_runtime_error)?;
            let output = f32_output_by_name_or_index(
                &outputs,
                &self.manifest.segmentation.output_name,
                0,
            )
    ...
    fn run_embeddings(&mut self, segmentations: &SegmentationBatch) -> Result<Vec<LocalSpeakerEmbedding>> {
        ...
        for (chunk, window) in segmentations.windows.iter().enumerate() {
            for local_speaker in 0..self.manifest.segmentation.local_speakers {
                ...
                let active_frames = mask.iter().filter(|value| **value > ACTIVE_THRESHOLD).count();
                ...
                let output = f32_output_by_name_or_index(
                    &outputs,
                    &self.manifest.embedding.output_name,
                    0,
                )
                    .or_else(|_| first_f32_output(&outputs))
                    .map_err(map_onnx_runtime_error)?;
...
fn resolve_config(config: PyannoteCommunityDiarizationConfig) -> Result<ResolvedPyannoteConfig> {
    ...
    let segmentation_model_path =
        require_file(&bundle_path.join(segmentation_model_file), "segmentation model")?;
    ...
    let clustering_config_path =
        require_file(&bundle_path.join(clustering_config_file), "clustering config")?;
...
fn speaker_mask(
    ...
    let copy_frames = manifest.segmentation.frames.min(manifest.embedding.mask_frames);
...
fn assign_local_speakers(
    ...
    let max_clusters = max_speakers.unwrap_or(embeddings.len()).min(embeddings.len()).max(1);
...
fn reconstruct_segments(
    ...
    let frame_duration = manifest.segmentation.duration_seconds / manifest.segmentation.frames as f64;
...
fn speaker_embedding_map(
    ...
    for assignment in assignments {
        if let Some(embedding) = embeddings
            .iter()
            .find(|item| item.chunk == assignment.chunk && item.local_speaker == assignment.local_speaker)
        {
            grouped.entry(assignment.speaker).or_default().push(embedding);
        }
    }
