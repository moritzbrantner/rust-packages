...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        ...
        Command::Operations { json } => {
            let payload =
                serde_json::to_string(&audio_analysis_pitch_cli::package_surface().operations)?;
            print_payload(json, "audio-analysis-pitch operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = audio_analysis_pitch_cli::run_operation(&operation, input)
