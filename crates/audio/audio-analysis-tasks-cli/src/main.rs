...
#[derive(Debug, Parser)]
#[command(
    name = "audio-analysis-tasks-cli",
    version,
    about = "Thin CLI adapter for audio-analysis-tasks"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            ...
            &audio_analysis_tasks_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "audio-analysis-tasks command schema",
            &audio_analysis_tasks_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload =
                serde_json::to_string(&audio_analysis_tasks_cli::package_surface().operations)?;
            print_payload(json, "audio-analysis-tasks operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = audio_analysis_tasks_cli::run_operation(&operation, input)
