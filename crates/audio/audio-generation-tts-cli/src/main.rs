...
#[derive(Debug, Parser)]
#[command(
    name = "audio-generation-tts-cli",
    version,
    about = "Thin CLI adapter for audio-generation-tts"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            ...
            &audio_generation_tts_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "audio-generation-tts command schema",
            &audio_generation_tts_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload =
                serde_json::to_string(&audio_generation_tts_cli::package_surface().operations)?;
            print_payload(json, "audio-generation-tts operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = audio_generation_tts_cli::run_operation(&operation, input)
