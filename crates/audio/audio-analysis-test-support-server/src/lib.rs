...
pub const LIBRARY_IMPORT: &str = "use audio_analysis_test_support";
/// Companion CLI package name.
pub const CLI_PACKAGE: &str = "audio-analysis-test-support-cli";
/// Companion React app package name.
pub const APP_PACKAGE: &str = "audio-analysis-test-support-app";
/// Companion WASM package name.
pub const WASM_PACKAGE: &str = "audio-analysis-test-support-wasm";
...
pub fn response_for(method: &str, path: &str, body: &str) -> HttpResponse {
    match (method, path) {
        ...
        ("GET", "/api/operations") => json_response(
            200,
            "OK",
            serde_json::json!(audio_analysis_test_support::surface::package_surface().operations),
        ),
...
fn schema_value() -> serde_json::Value {
    let operations = audio_analysis_test_support::surface::package_surface()
        .operations
        .into_iter()
        .map(|operation| {
...
fn run_request(request: SurfaceRequest) -> HttpResponse {
    match audio_analysis_test_support::surface::run_surface_operation(request) {
