...
#[derive(Debug, Parser)]
#[command(
    name = "audio-analysis-speakers-cli",
    version,
    about = "Thin CLI adapter for audio-analysis-speakers"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            ...
            &audio_analysis_speakers_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "audio-analysis-speakers command schema",
            &audio_analysis_speakers_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload =
                serde_json::to_string(&audio_analysis_speakers_cli::package_surface().operations)?;
            print_payload(json, "audio-analysis-speakers operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = audio_analysis_speakers_cli::run_operation(&operation, input)
