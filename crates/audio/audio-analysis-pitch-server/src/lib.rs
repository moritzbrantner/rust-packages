...
pub const LIBRARY_CRATE: &str = "audio-analysis-pitch";
/// Adapter surface kind.
pub const SURFACE_KIND: &str = "api";
/// Rust import path for the wrapped crate.
pub const LIBRARY_IMPORT: &str = "use audio_analysis_pitch";
/// Companion CLI package name.
pub const CLI_PACKAGE: &str = "audio-analysis-pitch-cli";
/// Companion React app package name.
pub const APP_PACKAGE: &str = "audio-analysis-pitch-app";
/// Companion WASM package name.
pub const WASM_PACKAGE: &str = "audio-analysis-pitch-wasm";
...
pub fn response_for(method: &str, path: &str, body: &str) -> HttpResponse {
    match (method, path) {
        ...
        ("GET", "/api/operations") => json_response(
            200,
            "OK",
            serde_json::json!(audio_analysis_pitch::surface::package_surface().operations),
        ),
...
fn schema_value() -> serde_json::Value {
    let operations = audio_analysis_pitch::surface::package_surface()
        .operations
        .into_iter()
        .map(|operation| {
...
fn run_request(request: SurfaceRequest) -> HttpResponse {
    match audio_analysis_pitch::surface::run_surface_operation(request) {
