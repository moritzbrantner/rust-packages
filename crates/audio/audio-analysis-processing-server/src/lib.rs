...
pub const LIBRARY_CRATE: &str = "audio-analysis-processing";
/// Adapter surface kind.
pub const SURFACE_KIND: &str = "api";
/// Rust import path for the wrapped crate.
pub const LIBRARY_IMPORT: &str = "use audio_analysis_processing";
/// Companion CLI package name.
pub const CLI_PACKAGE: &str = "audio-analysis-processing-cli";
/// Companion React app package name.
pub const APP_PACKAGE: &str = "audio-analysis-processing-app";
/// Companion WASM package name.
pub const WASM_PACKAGE: &str = "audio-analysis-processing-wasm";
...
pub fn response_for(method: &str, path: &str, body: &str) -> HttpResponse {
    match (method, path) {
        ...
        ("GET", "/api/operations") => json_response(
            200,
            "OK",
            serde_json::json!(audio_analysis_processing::surface::package_surface().operations),
        ),
...
fn package_metadata_value() -> serde_json::Value {
    let surface = audio_analysis_processing::surface::package_surface();
    serde_json::json!({
        "package": format!("{}-server", LIBRARY_CRATE),
...
fn schema_value() -> serde_json::Value {
    let operations = audio_analysis_processing::surface::package_surface()
        .operations
        .into_iter()
        .map(|operation| {
...
fn run_request(request: SurfaceRequest) -> HttpResponse {
    match audio_analysis_processing::surface::run_surface_operation(request) {
