...
#[derive(Debug, Parser)]
#[command(
    name = "video-analysis-mvs-cli",
    version,
    about = "Thin CLI adapter for video-analysis-mvs"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            json,
            "video-analysis-mvs",
            &video_analysis_mvs_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "video-analysis-mvs command schema",
            &video_analysis_mvs_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload =
                serde_json::to_string(&video_analysis_mvs_cli::package_surface().operations)?;
            print_payload(json, "video-analysis-mvs operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = video_analysis_mvs_cli::run_operation(&operation, input)
