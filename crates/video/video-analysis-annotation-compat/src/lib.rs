...
/// Converts the retained video-analysis dataset into the neutral annotation model.
///
/// The adapter extracts common semantics into the annotation envelope and also
/// preserves every complete legacy record as a JSON payload.
pub fn annotation_dataset_from_video_dataset(dataset: &AnalysisDataset) -> Result<AnnotationDataset> {
...
fn record_selector(record: &DatasetRecord) -> Option<AnnotationSelector> {
    match record {
        ...
        DatasetRecord::Feature(record) => record
            ...
            .or_else(|| {
                record.frame_index.map(|frame_index| AnnotationSelector::Frame {
                    frame_index,
                })
            }),
...
#[cfg(test)]
mod tests {
    use media_annotations::{AnnotationTiming, AnnotationValue};
    ...
    fn converts_legacy_events_losslessly_into_neutral_annotations() {
        ...
        assert!(matches!(annotation.timing, Some(AnnotationTiming::Instant(_))));
        assert!(annotation
            .start_timestamp()
            .unwrap()
