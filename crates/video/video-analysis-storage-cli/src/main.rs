...
#[derive(Debug, Parser)]
#[command(
    name = "video-analysis-storage-cli",
    version,
    about = "Thin CLI adapter for video-analysis-storage"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            ...
            &video_analysis_storage_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "video-analysis-storage command schema",
            &video_analysis_storage_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload =
                serde_json::to_string(&video_analysis_storage_cli::package_surface().operations)?;
            print_payload(json, "video-analysis-storage operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = video_analysis_storage_cli::run_operation(&operation, input)
