...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        ...
        Command::Operations { json } => {
            let payload =
                serde_json::to_string(&video_analysis_transform_cli::package_surface().operations)?;
            print_payload(json, "video-analysis-transform operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = video_analysis_transform_cli::run_operation(&operation, input)
