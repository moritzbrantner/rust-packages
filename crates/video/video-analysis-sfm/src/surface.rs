...
const SFM_MATCH_PLAN_OPERATION: &str = "video.sfm.matchPlan";
const SFM_CAMERA_GRAPH_OPERATION: &str = "video.sfm.cameraGraph";
const SFM_RECONSTRUCTION_SUMMARY_OPERATION: &str = "video.sfm.reconstructionSummary";
const SFM_RUST_MATCH_SUMMARY_OPERATION: &str = "video.sfmRust.matchSummary";
const SFM_RUST_TRACK_PLAN_OPERATION: &str = "video.sfmRust.trackPlan";
const SFM_RUST_BUNDLE_PLAN_OPERATION: &str = "video.sfmRust.bundlePlan";
const OPENCV_SFM_PLAN_OPERATION: &str = "video.opencv.sfmPlan";
    ...
    PackageSurface {
        ...
        operations: vec![
            operation(
                "describe",
                "Describe package",
                "Structure-from-motion planning, provider adapters, and sparse reconstruction contracts.",
                serde_json::json!({
            ...
            operation(
                ...
                "Plan SFM matching",
                "Builds a deterministic feature matching plan across extracted frames.",
                serde_json::json!({
                    "input": {},
                    "mode": "deterministic"
                }),
            ...
            operation(
                SFM_CAMERA_GRAPH_OPERATION,
                "Summarize camera graph",
                "Summarizes camera connectivity, frame pairs, and track coverage.",
                serde_json::json!({
                    "input": {},
                    "mode": "deterministic"
                }),
            ...
            operation(
                SFM_RECONSTRUCTION_SUMMARY_OPERATION,
                "Summarize SFM reconstruction",
                "Summarizes sparse cameras, points, observations, and reprojection diagnostics.",
                serde_json::json!({
                    "input": {},
                    "mode": "deterministic"
                }),
            ...
            operation(
                SFM_RUST_MATCH_SUMMARY_OPERATION,
                "Summarize Rust SFM matches",
                "Summarizes feature matches, image pairs, and deterministic matching thresholds.",
                serde_json::json!({
                    "input": {},
                    "mode": "deterministic"
                }),
            ...
            operation(
                SFM_RUST_TRACK_PLAN_OPERATION,
                "Plan Rust SFM tracks",
                "Builds a deterministic observation track construction plan.",
                serde_json::json!({
                    "input": {},
                    "mode": "deterministic"
                }),
            ...
            operation(
                SFM_RUST_BUNDLE_PLAN_OPERATION,
                "Plan Rust bundle adjustment",
                "Describes bundle adjustment variables, constraints, and convergence settings.",
                serde_json::json!({
                    "input": {},
                    "mode": "deterministic"
                }),
            ...
            operation(
                OPENCV_SFM_PLAN_OPERATION,
                "Plan OpenCV SFM",
                "Describes the optional OpenCV SfM provider setup without loading native bindings.",
                serde_json::json!({
                    "input": {},
                    "mode": "deterministic"
                }),
...
fn landscape_contract(id: &str) -> Option<runtime_core::landscape::LandscapeOperationContract> {
    match id {
        SFM_MATCH_PLAN_OPERATION => Some(runtime_core::landscape::LandscapeOperationContract::new(
            ...
            .input(
                runtime_core::landscape::LandscapePort::new(
                    "frames",
                    runtime_core::landscape::well_known::video_frame(),
                )
                .many(),
            )
            .output(runtime_core::landscape::LandscapePort::new(
                "matchPlan",
                runtime_core::landscape::well_known::video_sfm_match_plan(),
            ))
...
fn native_operation_diagnostic(operation: OperationId) -> SurfaceResponse {
    let message = format!(
        "`{RECONSTRUCT_VIDEO_OPERATION}` is a native server-only operation; call the video-analysis-sfm server or overview server dispatch path"
    );
    let result = serde_json::json!({
    ...
    let stages = vec![
        ...
        command_stage(
            ...
            vec![
                ...
                "--database_path".to_string(),
                path_string(&database_path),
                "--FeatureMatching.use_gpu".to_string(),
                "0".to_string(),
            ],
        ),
        command_stage(
