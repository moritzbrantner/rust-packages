...
#[derive(Debug, Parser)]
#[command(
    name = "video-analysis-reconstruction-cli",
    version,
    about = "Thin CLI adapter for video-analysis-reconstruction"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            ...
            &video_analysis_reconstruction_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "video-analysis-reconstruction command schema",
            &video_analysis_reconstruction_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload = serde_json::to_string(
                &video_analysis_reconstruction_cli::package_surface().operations,
            )?;
            print_payload(json, "video-analysis-reconstruction operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = video_analysis_reconstruction_cli::run_operation(&operation, input)
