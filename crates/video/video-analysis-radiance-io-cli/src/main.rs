...
#[derive(Debug, Parser)]
#[command(
    name = "video-analysis-radiance-io-cli",
    version,
    about = "Thin CLI adapter for video-analysis-radiance-io"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            ...
            &video_analysis_radiance_io_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "video-analysis-radiance-io command schema",
            &video_analysis_radiance_io_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload = serde_json::to_string(
                &video_analysis_radiance_io_cli::package_surface().operations,
            )?;
            print_payload(json, "video-analysis-radiance-io operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = video_analysis_radiance_io_cli::run_operation(&operation, input)
