...
#[derive(Debug, Parser)]
#[command(
    name = "video-analysis-opencv-backend-cli",
    version,
    about = "Thin CLI adapter for video-analysis-opencv-backend"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            ...
            &video_analysis_opencv_backend_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "video-analysis-opencv-backend command schema",
            &video_analysis_opencv_backend_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload = serde_json::to_string(
                &video_analysis_opencv_backend_cli::package_surface().operations,
            )?;
            print_payload(json, "video-analysis-opencv-backend operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = video_analysis_opencv_backend_cli::run_operation(&operation, input)
