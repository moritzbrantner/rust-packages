...
#[derive(Debug, Parser)]
#[command(
    name = "video-analysis-gaussian-splatting-cli",
    version,
    about = "Thin CLI adapter for video-analysis-gaussian-splatting"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            ...
            &video_analysis_gaussian_splatting_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "video-analysis-gaussian-splatting command schema",
            &video_analysis_gaussian_splatting_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload = serde_json::to_string(
                &video_analysis_gaussian_splatting_cli::package_surface().operations,
            )?;
            print_payload(
                json,
                "video-analysis-gaussian-splatting operations",
                &payload,
            );
        }
        Command::Run {
            ...
            let response = video_analysis_gaussian_splatting_cli::run_operation(&operation, input)
