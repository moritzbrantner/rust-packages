...
pub const LIBRARY_CRATE: &str = "video-analysis-opencv-backend";
/// Adapter surface kind.
pub const SURFACE_KIND: &str = "api";
/// Rust import path for the wrapped crate.
pub const LIBRARY_IMPORT: &str = "use video_analysis_opencv_backend";
/// Companion CLI package name.
pub const CLI_PACKAGE: &str = "video-analysis-opencv-backend-cli";
/// Companion React app package name.
pub const APP_PACKAGE: &str = "video-analysis-opencv-backend-app";
/// Companion WASM package name.
pub const WASM_PACKAGE: &str = "video-analysis-opencv-backend-wasm";
...
pub fn response_for(method: &str, path: &str, body: &str) -> HttpResponse {
    match (method, path) {
        ...
        ("GET", "/api/operations") => json_response(
            200,
            "OK",
            serde_json::json!(video_analysis_opencv_backend::surface::package_surface().operations),
        ),
...
fn schema_value() -> serde_json::Value {
    let operations = video_analysis_opencv_backend::surface::package_surface()
        .operations
        .into_iter()
        .map(|operation| {
...
fn run_request(request: SurfaceRequest) -> HttpResponse {
    match video_analysis_opencv_backend::surface::run_surface_operation(request) {
