...
#[derive(Debug, Parser)]
#[command(
    name = "video-analysis-output-cli",
    version,
    about = "Thin CLI adapter for video-analysis-output"
)]
struct Cli {
    #[command(subcommand)]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.unwrap_or(Command::Info { json: false }) {
        Command::Info { json } => print_payload(
            ...
            &video_analysis_output_cli::package_metadata_json(),
        ),
        Command::Schema { json } => print_payload(
            json,
            "video-analysis-output command schema",
            &video_analysis_output_cli::command_schema_json(),
        ),
        Command::Operations { json } => {
            let payload =
                serde_json::to_string(&video_analysis_output_cli::package_surface().operations)?;
            print_payload(json, "video-analysis-output operations", &payload);
        }
        Command::Run {
            operation,
            json,
            ...
            let response = video_analysis_output_cli::run_operation(&operation, input)
