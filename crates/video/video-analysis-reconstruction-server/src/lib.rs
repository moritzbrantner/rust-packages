...
pub const LIBRARY_CRATE: &str = "video-analysis-reconstruction";
/// Adapter surface kind.
pub const SURFACE_KIND: &str = "api";
/// Rust import path for the wrapped crate.
pub const LIBRARY_IMPORT: &str = "use video_analysis_reconstruction";
/// Companion CLI package name.
pub const CLI_PACKAGE: &str = "video-analysis-reconstruction-cli";
/// Companion React app package name.
pub const APP_PACKAGE: &str = "video-analysis-reconstruction-app";
/// Companion WASM package name.
pub const WASM_PACKAGE: &str = "video-analysis-reconstruction-wasm";
...
pub fn response_for(method: &str, path: &str, body: &str) -> HttpResponse {
    match (method, path) {
        ...
        ("GET", "/api/operations") => json_response(
            200,
            "OK",
            serde_json::json!(video_analysis_reconstruction::surface::package_surface().operations),
        ),
...
fn package_metadata_value() -> serde_json::Value {
    let surface = video_analysis_reconstruction::surface::package_surface();
    serde_json::json!({
        "package": format!("{}-server", LIBRARY_CRATE),
...
fn schema_value() -> serde_json::Value {
    let operations = video_analysis_reconstruction::surface::package_surface()
        .operations
        .into_iter()
        .map(|operation| {
...
fn run_request(request: SurfaceRequest) -> HttpResponse {
    match video_analysis_reconstruction::surface::run_surface_operation(request) {
