...
pub const LIBRARY_CRATE: &str = "video-analysis-sfm-rust-backend";
/// Adapter surface kind.
pub const SURFACE_KIND: &str = "api";
/// Rust import path for the wrapped crate.
pub const LIBRARY_IMPORT: &str = "use video_analysis_sfm_rust_backend";
/// Companion CLI package name.
pub const CLI_PACKAGE: &str = "video-analysis-sfm-rust-backend-cli";
/// Companion React app package name.
pub const APP_PACKAGE: &str = "video-analysis-sfm-rust-backend-app";
/// Companion WASM package name.
pub const WASM_PACKAGE: &str = "video-analysis-sfm-rust-backend-wasm";
...
pub fn response_for(method: &str, path: &str, body: &str) -> HttpResponse {
    match (method, path) {
        ...
        ("GET", "/api/operations") => json_response(
            ...
            "OK",
            serde_json::json!(
                video_analysis_sfm_rust_backend::surface::package_surface().operations
            ),
        ),
...
fn package_metadata_value() -> serde_json::Value {
    let surface = video_analysis_sfm_rust_backend::surface::package_surface();
    serde_json::json!({
        "package": format!("{}-server", LIBRARY_CRATE),
...
fn schema_value() -> serde_json::Value {
    let operations = video_analysis_sfm_rust_backend::surface::package_surface()
        .operations
        .into_iter()
        .map(|operation| {
...
fn run_request(request: SurfaceRequest) -> HttpResponse {
    match video_analysis_sfm_rust_backend::surface::run_surface_operation(request) {
